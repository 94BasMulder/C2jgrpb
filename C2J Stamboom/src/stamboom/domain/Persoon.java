...
public class Persoon {
    ...
    Persoon(int persNr, String[] vnamen, String anaam, String tvoegsel,
        ...
        {
            vnaam =  Character.toUpperCase(vnaam.charAt(0)) + vnaam.substring(1).toLowerCase();
        }
        ...
        alsOuderBetrokkenIn = getAlsOuderBetrokkenIn();
    }
   ...
   public Gezin heeftOngehuwdGezinMet(Persoon andereOuder) {
        ...
        if (this.getGebDat().compareTo(Calendar.getInstance()) > 0) {
            return null;
        }
        if (andereOuder != null && andereOuder.getGebDat().compareTo(Calendar.getInstance()) > 0) {
            return null;
        }

        Calendar nu = Calendar.getInstance();
        if (this.isGetrouwdOp(nu) || (andereOuder != null
                && andereOuder.isGetrouwdOp(nu))
                ) {
            return null;
        }
        Random rand = new Random();
        int nextGezinsNr = rand.nextInt(1000) +1;
        return new Gezin(nextGezinsNr, this, andereOuder);
    }
