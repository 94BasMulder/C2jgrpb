...
public class Administratie {
    ...
    public Persoon addPersoon(Geslacht geslacht, String[] vnamen, String anaam,
        ...
        for(Persoon p: personen)
            if(p.getNaam().equalsIgnoreCase(initialen + " " + tvoegsel + " " + anaam))
                if(p.getGebPlaats().equals(gebplaats))
                    if(p.getGebDat().equals(gebdat))
        return null;
        return new Persoon(nextPersNr, vnamen, anaam, tvoegsel, gebdat, gebplaats, geslacht, ouderlijkGezin);
    }
    ...
    public Gezin addHuwelijk(Persoon ouder1, Persoon ouder2, Calendar huwdatum) {
        for(Gezin g : gezinnen)
            if(g.getHuwelijksdatum().equals(null) && ouder1.equals(ouder2))
            {
                g.setHuwelijk(huwdatum);
                return g;
            }
            else if(g.getHuwelijksdatum() != null)
            {
                return g;
            }
        return null;
